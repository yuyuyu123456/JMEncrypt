...
static int start_slice(Slice *currSlice, StatParameters *cur_stats)
{
  ...
  for (i = 0; i < NumberOfPartitions; i++)
  {
	...
	for (int i = 0; i < 26; i++) {
		printf("%x ",currStream->streamBuffer[i] );
	}
    //! Initialize CABAC
...
/*
 shuffle index array
*/
void shuffleIndex(int count, int** datashuffle) {
	int r, m;
	for (int i = 0; i < count - 2; i++) {
		r = rand()%(count-i-2)+i+1;
	    m = *datashuffle[i];
		*datashuffle[i] = *datashuffle[r];
		*datashuffle[r] = m;
	   }
	m = *datashuffle[count - 2];
	*datashuffle[count - 2] =*datashuffle[count-1];
	*datashuffle[count - 1] = m;
...
int encode_one_slice(VideoParameters *p_Vid, int SliceGroupId, int TotalCodedMBs)
{
	...
	int bits_to_go = currSlice->partArr[0].bitstream->bits_to_go;
	int byte_pos = currSlice->partArr[0].bitstream->byte_pos;
	int bits_to_go_skip = currSlice->partArr[0].bitstream->bits_to_go_skip;
	int buffersize = currSlice->partArr[0].bitstream->buffer_size;
	byte byte_buf = currSlice->partArr[0].bitstream->byte_buf;
	byte byte_buf_skip = currSlice->partArr[0].bitstream->byte_buf_skip;
	int byte_pos_skip = currSlice->partArr[0].bitstream->byte_pos_skip;
	int stored_bits_to_go=currSlice->partArr[0].bitstream->stored_bits_to_go;
	byte    stored_byte_buf = currSlice->partArr[0].bitstream->stored_byte_buf;
	int     stored_byte_pos = currSlice->partArr[0].bitstream->stored_byte_pos;

	//CSobj obj = *(currSlice->p_RDO->cs_cm);
	//int  i_last = currSlice->idr_flag ? 1 : cs->no_part;

	//=== important variables of data partition array ===
	/*for (i = 0; i < i_last; i++)
	{*/
	/*obj.bitstream[0].bits_to_go = currSlice->partArr[0].bitstream->bits_to_go;
	obj.bitstream[0].byte_pos = currSlice->partArr[0].bitstream->byte_pos;
	obj.bitstream[0].buffer_size = currSlice->partArr[0].bitstream->buffer_size;
	obj.bitstream[0].stored_byte_pos = currSlice->partArr[0].bitstream->stored_byte_pos;
	obj.bitstream[0].stored_bits_to_go = currSlice->partArr[0].bitstream->stored_bits_to_go;
	obj.bitstream[0].byte_pos_skip = currSlice->partArr[0].bitstream->byte_pos_skip;
	obj.bitstream[0].bits_to_go_skip = currSlice->partArr[0].bitstream->bits_to_go_skip;
	obj.bitstream[0].write_flag = currSlice->partArr[0].bitstream->write_flag;
	obj.bitstream[0].byte_buf = currSlice->partArr[0].bitstream->byte_buf;
	obj.bitstream[0].stored_byte_buf = currSlice->partArr[0].bitstream->stored_byte_buf;
	obj.bitstream[0].byte_buf_skip =currSlice->partArr[0].bitstream->byte_buf_skip;
	obj.bitstream[0].streamBuffer= currSlice->partArr[0].bitstream->streamBuffer;*/
	//}
		...
		else
		{
			...
			for (int i = 0; i < BLOCK_SIZE + currMB->p_Vid->num_blk8x8_uv; i++)
				for (int j = 0; j < BLOCK_SIZE; j++)
					for (int k = 0; k < 2; k++)
					{
						...
						for (int l = 0; l < 65; l++) {
							currSlice->cofAC_write[currMB->mb_x][currMB->mb_y][i][j][k][l] = currSlice->cofAC[i][j][k][l];
							//printf("%d %d %d %d  %d  %d \n", i, j, k, l, currSlice->cofAC_write[currMB->mb_x][currMB->mb_y][i][j][k][l], currSlice->cofAC[i][j][k][l]);
						}
			...
			for (int i = 0; i < 3; i++)
				for (int j = 0; j < 2; j++) {
					...
					for (int l = 0; l < 18; l++) {
						//printf("old %d %d %d  %d  %d \n", i,j,l, currSlice->cofDC_write[currMB->mb_x][currMB->mb_y][i][j][l], currSlice->cofDC[i][j][l]);
						currSlice->cofDC_write[currMB->mb_x][currMB->mb_y][i][j][l] = currSlice->cofDC[i][j][l];
			...
			//currSlice->write_MB_layer(currMB, 0, &i);
			printf("\n\n");
		}
 ...
 if (currSlice->slice_type == 2) {
	int *index;
	index = (int*)malloc(NumberOfCodedMBs * sizeof(int));
	for (int i = 0; i < NumberOfCodedMBs; i++) {
		index[i] = i;
	}
	int **datashuffle;
	get_mem2Dint(&datashuffle, (p_Vid->PicWidthInMbs - 2)*(p_Vid->FrameHeightInMbs - 2), 1);
	int count = 0;
	for (int i = 1; i < p_Vid->FrameHeightInMbs - 1; i++)
		for (int j = 1; j < p_Vid->PicWidthInMbs - 1; j++) {
			datashuffle[count++] = &index[p_Vid->PicWidthInMbs*i + j];
		}
	shuffleIndex(count, datashuffle);
	count = 0;
	for (int i = 1; i < p_Vid->PicWidthInMbs - 1; i++)
		datashuffle[count++] = &index[i];
	shuffleIndex(count, datashuffle);
	count = 0;
	for (int i = 1; i < p_Vid->PicWidthInMbs - 1; i++)
		datashuffle[count++] = &index[i + (p_Vid->FrameHeightInMbs - 1)*p_Vid->PicWidthInMbs];
	shuffleIndex(count, datashuffle);
	count = 0;
	for (int i = 1; i < p_Vid->FrameHeightInMbs - 1; i++)
		datashuffle[count++] = &index[i*p_Vid->PicWidthInMbs];
	shuffleIndex(count, datashuffle);
	count = 0;
	for (int i = 1; i < p_Vid->FrameHeightInMbs - 1; i++)
		datashuffle[count++] = &index[i*p_Vid->PicWidthInMbs + p_Vid->PicWidthInMbs - 1];
	shuffleIndex(count, datashuffle);
	Macroblock*mb_data;
	if((mb_data= alloc_mbs(p_Vid, p_Vid->FrameSizeInMbs, p_Vid->num_of_layers)) == NULL){
	  no_mem_exit("init_img: p_Vid->mb_data");
     }
	for (int i = 0; i < NumberOfCodedMBs; i++) {
	printf("%d ", index[i]);
	mb_data[i] = p_Vid->mb_data[index[i]];
}
	//printf("%d %d %d", obj.bits.mb_total, obj.bitstream->bits_to_go, obj.bitstream->byte_pos);
	//printf("%x ", obj.bitstream->streamBuffer);
	/*if ((currSlice->partArr = (DataPartition *)calloc(currSlice->max_part_nr, sizeof(DataPartition))) == NULL)
		...
		currSlice->partArr[0].bitstream->bits_to_go = bits_to_go;
		currSlice->partArr[0].bitstream->bits_to_go_skip = bits_to_go_skip;
		currSlice->partArr[0].bitstream->buffer_size = buffersize;
		currSlice->partArr[0].bitstream->byte_buf = byte_buf;
		currSlice->partArr[0].bitstream->byte_buf_skip = byte_buf_skip;
		currSlice->partArr[0].bitstream->byte_pos = byte_pos;
		currSlice->partArr[0].bitstream->byte_pos_skip = byte_pos_skip;
		currSlice->partArr[0].bitstream->stored_bits_to_go = stored_bits_to_go;
		currSlice->partArr[0].bitstream->stored_byte_buf = stored_byte_buf;
		currSlice->partArr[0].bitstream->stored_byte_pos = stored_byte_pos;
		byte    *streamBuffer;
		if ((streamBuffer = (byte *)calloc(buffer_size, sizeof(byte))) == NULL)
			no_mem_exit("malloc_slice: StreamBuffer");
		for (int i = 0; i < len; i++) {
			streamBuffer[i] = currSlice->partArr[0].bitstream->streamBuffer[i];
		}
		currSlice->partArr[0].bitstream->streamBuffer = streamBuffer;
		/*for (int i = 0; i < currSlice->max_part_nr; i++)
		{
			DataPartition *dataPart = &(currSlice->partArr[i]);

			Bitstream* currStream = dataPart->bitstream;
			currStream->bits_to_go = 8;
			currStream->byte_pos = 0;
			currStream->byte_buf = 0;
		}*/
		currSlice->p_RDO->cs_cm->bitstream[0] = *currSlice->partArr[0].bitstream;
		p_Vid->mb_data = mb_data;
		for (int i = 0; i < NumberOfCodedMBs; i++) {
			...
			Macroblock* cur = &p_Vid->mb_data[i];
			cur->bits.mb_cbp = 0;
			cur->bits.mb_cb_coeff = 0;
			cur->bits.mb_cr_coeff = 0;
			cur->bits.mb_delta_quant = 0;
			cur->bits.mb_inter = 0;
			cur->bits.mb_mode = 0;
			cur->bits.mb_stuffing = 0;
			cur->bits.mb_total = 0;
			cur->bits.mb_uv_coeff = 0;
			cur->bits.mb_y_coeff = 0;
			currSlice->writeCoeff4x4_CAVLC = writeCoeff4x4_CAVLC_normal_all_write;
			//encryptdctflag = 1;
			if (i%p_Vid->PicWidthInMbs > 0)
				cur->mb_left = &p_Vid->mb_data[i - 1];
			if (i >= p_Vid->PicWidthInMbs)
				cur->mb_up = &p_Vid->mb_data[i - p_Vid->PicWidthInMbs];
			cur->mbAddrX = i;
			if (cur->mbAvailA)
				cur->mbAddrA = p_Vid->mb_data[i-1].mbAddrX;
			if (cur->mbAvailB)
				cur->mbAddrB = p_Vid->mb_data[i - p_Vid->PicWidthInMbs].mbAddrX;
			if (cur->mbAvailC)
				cur->mbAddrC = p_Vid->mb_data[i - p_Vid->PicWidthInMbs + 1].mbAddrX;
			if (cur->mbAvailD)
				cur->mbAddrD = p_Vid->mb_data[i - p_Vid->PicWidthInMbs - 1].mbAddrX;
			if (i > 0) {
				cur->PrevMB = &p_Vid->mb_data[i - 1];
				cur->prev_qp = cur->PrevMB->qp;
				cur->prev_dqp = cur->prev_qp - cur->PrevMB->prev_qp;
			}
			else
			{
                cur->PrevMB = NULL;
				cur->prev_qp = (short)currSlice->qp;
				cur->prev_dqp = 0;
			}
			
			cur->mb_x = cur->mbAddrX%p_Vid->PicWidthInMbs;
			cur->mb_y = cur->mbAddrX / p_Vid->PicWidthInMbs;
			/*if (i-1 > -1 && (p_Vid->mb_data[index[i-1]].slice_nr == currSlice->slice_nr))
			{
				cur.PrevMB = &p_Vid->mb_data[index[i - 1]];
				cur.prev_qp = cur.PrevMB->qp;
				cur.prev_dqp = cur.prev_qp - cur.PrevMB->prev_qp;
			}
			else
...
void free_slice(Slice *currSlice)
{
  if (currSlice != NULL)
  {
	...
	if (currSlice->cofAC_write)
		free_mem_ACcoeffwrite(currSlice->cofAC_write);
	if (currSlice->cofDC_write)
		free_mem_DCcoeffwrite(currSlice->cofDC_write);
